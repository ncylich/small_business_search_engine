...
def df_messages(service, email_list, user_id='me', output_file='brandon_labels.csv'):
    ...
    for email, from_msgs, to_msgs in zip(email_list, messages_from, messages_to):
        from_msgs = {f'from_{i + 1}': get_email_string(service, user_id, m['id']) for i, m in enumerate(from_msgs)}
        to_msgs = {f'to_{i + 1}': get_email_string(service, user_id, m['id']) for i, m in enumerate(to_msgs)}
        all_msgs = from_msgs | to_msgs
        data[email] = {'email': email} | all_msgs  # add email to the dictionary
